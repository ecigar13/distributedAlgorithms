...
public class SlaveThread implements Runnable 
{
   ...
   * Reciever_id:Message;
   */
  //StringBuilder msg_to_be_sent = new StringBuilder();
  protected int leaderId = 0;			//remove later
  protected int diam = 0;				//remove later
		  ...
		  temp_priority_queue = masterNode.Data_Messages.get(this.sno_in_graph);
			  ...
			  while(!temp_priority_queue.isEmpty())
			  {
				  ...
				  if(temp_message_var.getmType().equals("Terminate"))
				  	...
				  	{
				  		System.out.println("If message recieved in thia round;inside 3 else");
