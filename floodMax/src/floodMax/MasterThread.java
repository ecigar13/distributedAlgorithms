...
public class MasterThread extends SlaveThread 
{
	...
	//maxid to be in sync with message class object. Not needed here. Junk value
  ...
  public void run() 
  {
 	...
 	temp_priority_queue.add(msg);
	    ...
	    for(int i = 1; i < size; i++)
	    {
	    	...
	    	//this.roundFinishStatus = RoundDone.NO;		//gunjan
	    }	  
	  
	  for(int i = 0; i <size; i++)
	  {
		  System.out.println("Size of threads is"+Data_Messages.get(i).size()+"Sno is "+ i);
	  }
		  ...
		  while(!(temp_priority_queue.isEmpty()))
		  {
			  ...
			  //System.out.println(temp_Message_obj.getmType());
			  	if(temp_Message_obj.getmType().equals("Leader"))
			  	{
			    	...
			    	msg = new Message(this.master_id,this.master_round, this.max_uid, "Terminate");
			    	temp_priority_queue.add(msg);
			  	...
			  	else if ( (temp_Message_obj.getmType().equals("Done")) && (temp_Message_obj.getRound() == this.master_round) )
			  	{
			  		...
			  		if (Done_Count == size)
			  		{
			  			...
			  			this.master_round++;
				    	msg = new Message(this.master_id,this.master_round, this.max_uid, "Round_Number");
				    	temp_priority_queue.add(msg);
				    	for (int i = 1; i< size;i++)
				    	{
				    		Data_Messages.put(i, temp_priority_queue);
