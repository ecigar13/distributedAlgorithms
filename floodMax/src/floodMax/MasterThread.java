...
public class MasterThread extends SlaveThread {
  ...
  public void run() {
    ...
    for (int i = 1; i < size; i++) {
      ...
      System.out.println("message is :" + temp_priority_queue.toString());
    ...
    while (true) {
      // master retrieves its queue from the queue outside.
      ...
      while (!(temp_priority_queue.isEmpty())) {
        ...
        if (temp_Message_obj.getmType().equals("Leader")) {
          ...
          //if the sender sent "Leader" message, then tell sender "Terminate". This sender is the leader. 
          //terminate is static so all threads will terminate.
        ...
        //if the thread says it's completed the round, increase the count.
        else if ((temp_Message_obj.getmType().equals("Done")) && (temp_Message_obj.getRound() == this.master_round)) {
          Done_Count++;
          //if all slaves completed the round
          if (Done_Count == size) {

            this.master_round++;
            Message msg = new Message(this.master_id, this.master_round, this.max_uid, "Round_Number");
            
            //send message to the queue of all threads to make signals.
            temp_priority_queue.add(msg);
            for (int i = 1; i < size; i++) {
              Data_Messages.put(i, temp_priority_queue);
            }
